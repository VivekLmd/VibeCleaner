...
# Rewrite all git history to replace a personal email with a noreply address.
# Usage (example):
#   OLD_EMAIL="you@example.com" NEW_EMAIL="anonymous@users.noreply.github.com" ./scripts/rewrite_email.sh
...
if [[ -z "${OLD_EMAIL}" || -z "${NEW_EMAIL}" ]]; then
  ...
  echo "  OLD_EMAIL=you@example.com NEW_EMAIL=anonymous@users.noreply.github.com ./scripts/rewrite_email.sh" >&2
  exit 1
fi
